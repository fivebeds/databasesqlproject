...
--Find theforename and surnames names of all employees
...
-- Unions (can't be multiple columns and same data type)

--Find a list of employee and branch names
SELECT first_name AS FIRST_AND_BRANCH FROM employee UNION SELECT branch_name FROM branch;
...
--LEFT JOIN includes everyone from the from statement, but still only includes the ones for on
SELECT employee.emp_id, employee.first_name, branch.branch_name
FROM employee
LEFT JOIN branch
ON employee.emp_id = branch.mgr_id;
...
--Nested Queries (starts the nested one and then does the outer statement)
...
--Find all clients who are handled by the branch that Micael Scott manages
...
-- cascade - primary, set null - foreign key
